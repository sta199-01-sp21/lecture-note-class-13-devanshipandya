...
The natural point estimate for the population mean would be p-hat.
...
point estimates are generated only from one population sample. It is not very
likely that this point statistic actually represents the actual value for the
population.
...
I would be surprised if 40 were left handed, but I would not be
surprised if 3 were left handed. 
...
What does this mean?
These means that in repeated samples, we can say with 95% confidence that the
interval contains the true mean for the population.
...
```
This means that we can say with 95% confidence that the median of prices of 1 bedroom apartments in manhattan
are between $2162 and $2875.
...
- Population: mice
- Parameter of interest: proportion that died while not on treatment
- Sample: mice
- Sample size: 522
...
What does this mean?
We can say with 95% confidence that between 69% and 79.7% of mice died while not on treatment
...
boot_mice <- mice %>% 
  filter(treat == 0) %>% 
  mutate(died = if_else(died == 0, "yes", "no")) %>% 
  specify(response = died, success = "yes") %>% 
  generate(reps = 10000, type = "bootstrap") %>% 
  calculate(stat = "prop") 
boot_mice
...
```{r practice_2_ci}
ci_mice_live <- get_ci(boot_mice, level = .95)
ci_mice_live
```
...
We can say with 95% confidence that the proprtion of mice that lived on treatment 
are between .203 and .310.
